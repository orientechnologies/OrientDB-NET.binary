...
namespace Orient.Tests
{
    public class DbRunner
    {
        ...
        private static bool IsOrientAlreadyListening()
        {
            const int orientPort = 2424;
        ...
        public static void StopOrientDb(string dbDir)
        {
            if (_process == null)
                return;

            var path = Path.Combine(dbDir, @"bin\shutdown.bat");
            Process.Start(path);
            Thread.Sleep(1000);
